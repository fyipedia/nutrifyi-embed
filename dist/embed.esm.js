/* nutrifyi-embed v1.0.0 | MIT | https://widget.nutrifyi.com */
...
function calculateBMI(heightCm, weightKg) {
  if (heightCm <= 0 || weightKg <= 0) {
    return { bmi: 0, category: "Invalid", color: "#64748b" };
  }
  const heightM = heightCm / 100;
  const bmi = weightKg / (heightM * heightM);
  const rounded = Math.round(bmi * 10) / 10;
  if (bmi < 18.5) return { bmi: rounded, category: "Underweight", color: "#3b82f6" };
  if (bmi < 25) return { bmi: rounded, category: "Normal", color: "#22c55e" };
  if (bmi < 30) return { bmi: rounded, category: "Overweight", color: "#f59e0b" };
  return { bmi: rounded, category: "Obese", color: "#ef4444" };
}
...
function dailyValuePct(nutrient, amount) {
  const dv = DAILY_VALUES[nutrient];
  if (!dv || amount <= 0) return 0;
...
function initBMICalculatorTool(el, config) {
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "healthfyi-bmi-widget");
  container.innerHTML = `
    <div class="healthfyi-header">
      <div>
        <div class="healthfyi-header-title">BMI Calculator</div>
        <div class="healthfyi-header-subtitle">Body Mass Index</div>
      </div>
    </div>
    <div style="padding:16px 18px;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
        <div>
          <label style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.04em;display:block;margin-bottom:4px;">Height (cm)</label>
          <input class="healthfyi-bmi-height healthfyi-search-input" type="number" min="50" max="300" step="1" placeholder="170"
                 style="width:100%;padding:8px 10px;border:1px solid var(--input-border);border-radius:6px;font-size:14px;background:var(--input-bg);color:var(--text);">
        </div>
        <div>
          <label style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:0.04em;display:block;margin-bottom:4px;">Weight (kg)</label>
          <input class="healthfyi-bmi-weight healthfyi-search-input" type="number" min="10" max="500" step="0.1" placeholder="70"
                 style="width:100%;padding:8px 10px;border:1px solid var(--input-border);border-radius:6px;font-size:14px;background:var(--input-bg);color:var(--text);">
        </div>
      </div>
      <button class="healthfyi-bmi-calc healthfyi-search-btn" style="width:100%;padding:10px;font-size:14px;font-weight:600;border-radius:6px;cursor:pointer;">
        Calculate BMI
      </button>
      <div class="healthfyi-bmi-result" style="margin-top:12px;display:none;"></div>
    </div>
    ${poweredByHTML(config)}
  `;
  const heightInput = container.querySelector(".healthfyi-bmi-height");
  const weightInput = container.querySelector(".healthfyi-bmi-weight");
  const calcBtn = container.querySelector(".healthfyi-bmi-calc");
  const resultDiv = container.querySelector(".healthfyi-bmi-result");
  function calculate() {
    const height = parseFloat(heightInput.value);
    const weight = parseFloat(weightInput.value);
    if (!height || !weight || height <= 0 || weight <= 0) {
      resultDiv.style.display = "block";
      resultDiv.innerHTML = `<div style="text-align:center;color:var(--muted);font-size:13px;">Please enter valid height and weight.</div>`;
      return;
    }
    const result = calculateBMI(height, weight);
    resultDiv.style.display = "block";
    resultDiv.innerHTML = `
      <div style="text-align:center;padding:12px;background:color-mix(in srgb, ${result.color} 8%, var(--bg));border-radius:8px;border:1px solid color-mix(in srgb, ${result.color} 20%, transparent);">
        <div style="font-size:32px;font-weight:800;color:${result.color};line-height:1;">${result.bmi}</div>
        <div style="font-size:14px;font-weight:600;color:${result.color};margin-top:4px;">${escapeHTML4(result.category)}</div>
        <div style="font-size:11px;color:var(--muted);margin-top:6px;">
          BMI Scale: Underweight &lt;18.5 &middot; Normal 18.5\u201324.9 &middot; Overweight 25\u201329.9 &middot; Obese &ge;30
        </div>
      </div>
    `;
  }
...
// src/tools/nutrition-label.ts
function escapeHTML5(str) {
  return str.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function dvRow(label, amount, unit, nutrientKey, bold = false) {
  if (amount === null || amount === void 0) return "";
  const dv = dailyValuePct(nutrientKey, amount);
  const dvStr = dv > 0 ? `${dv}%` : "";
  const nameStyle = bold ? "font-weight:700;" : "";
  return `
    <div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid var(--border);">
      <span style="font-size:12px;${nameStyle}">${label} <span style="font-weight:400;">${amount}${unit}</span></span>
      <span style="font-size:12px;font-weight:700;">${dvStr}</span>
    </div>
  `;
}
function subDvRow(label, amount, unit, nutrientKey) {
  if (amount === null || amount === void 0) return "";
  const dv = dailyValuePct(nutrientKey, amount);
  const dvStr = dv > 0 ? `${dv}%` : "";
  return `
    <div style="display:flex;justify-content:space-between;padding:2px 0 2px 16px;border-bottom:1px solid var(--border);">
      <span style="font-size:11px;">${label} ${amount}${unit}</span>
      <span style="font-size:11px;font-weight:700;">${dvStr}</span>
    </div>
  `;
}
function initNutritionLabelTool(el, config) {
  var _a;
  const slug = (_a = el.dataset.slug) != null ? _a : "";
  if (!slug) {
    const shadow2 = createShadow(el, config);
    const container2 = createWidgetRoot(shadow2, el, "healthfyi-label-widget");
    renderError(container2, "Missing data-slug attribute.", config);
    return;
  }
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el, "healthfyi-label-widget");
  renderLoading(container);
  fetchAPI(config.apiBase, `nutrition/`, { food_slug: slug }).then((rawData) => {
    var _a2, _b, _c;
    const data = Array.isArray(rawData) ? rawData[0] : rawData;
    if (!data) {
      renderError(container, `No nutrition data for "${escapeHTML5(slug)}".`, config);
      return;
    }
    const foodName = (_a2 = data.food_name) != null ? _a2 : slug.replace(/-/g, " ");
    const serving = (_b = data.serving_description) != null ? _b : data.serving_size_g ? `${data.serving_size_g}g` : "";
    const calories = (_c = data.calories) != null ? _c : 0;
    const foodUrl = `https://${config.domain}/food/${escapeHTML5(slug)}/`;
    container.innerHTML = `
        <div style="padding:12px 18px;border-bottom:8px solid var(--text);">
          <div style="font-size:24px;font-weight:900;line-height:1;">Nutrition Facts</div>
          ${serving ? `<div style="font-size:12px;color:var(--muted);margin-top:4px;">Serving: ${escapeHTML5(serving)}</div>` : ""}
        </div>
        <div style="padding:4px 18px;border-bottom:1px solid var(--border);">
          <div style="display:flex;justify-content:space-between;align-items:baseline;">
            <span style="font-size:12px;font-weight:700;">Calories</span>
            <span style="font-size:28px;font-weight:900;">${calories}</span>
          </div>
        </div>
        <div style="padding:2px 18px 0;">
          <div style="text-align:right;font-size:10px;font-weight:700;padding:4px 0;border-bottom:1px solid var(--border);">% Daily Value*</div>
          ${dvRow("Total Fat", data.total_fat_g, "g", "total_fat_g", true)}
          ${subDvRow("Saturated Fat", data.saturated_fat_g, "g", "saturated_fat_g")}
          ${data.trans_fat_g != null ? subDvRow("Trans Fat", data.trans_fat_g, "g", "") : ""}
          ${dvRow("Cholesterol", data.cholesterol_mg, "mg", "cholesterol_mg", true)}
          ${dvRow("Sodium", data.sodium_mg, "mg", "sodium_mg", true)}
          ${dvRow("Total Carbohydrate", data.total_carbohydrate_g, "g", "total_carbohydrate_g", true)}
          ${subDvRow("Dietary Fiber", data.dietary_fiber_g, "g", "dietary_fiber_g")}
          ${subDvRow("Sugars", data.sugars_g, "g", "sugars_g")}
          ${dvRow("Protein", data.protein_g, "g", "protein_g", true)}
        </div>
        <div style="padding:4px 18px 8px;border-top:8px solid var(--text);">
          ${data.vitamin_d_mcg != null ? dvRow("Vitamin D", data.vitamin_d_mcg, "mcg", "vitamin_d_mcg") : ""}
          ${data.calcium_mg != null ? dvRow("Calcium", data.calcium_mg, "mg", "calcium_mg") : ""}
          ${data.iron_mg != null ? dvRow("Iron", data.iron_mg, "mg", "iron_mg") : ""}
          ${data.potassium_mg != null ? dvRow("Potassium", data.potassium_mg, "mg", "potassium_mg") : ""}
          <div style="font-size:9px;color:var(--muted);margin-top:6px;line-height:1.3;">
            * Percent Daily Values based on a 2,000 calorie diet.
          </div>
        </div>
        <div class="healthfyi-view-link">
          <a href="${escapeHTML5(foodUrl)}" target="_blank" rel="noopener" style="color:${config.accent};text-decoration:none;font-size:12px;font-weight:500;">
            ${escapeHTML5(foodName)} on ${escapeHTML5(config.name)} \u2197
          </a>
        </div>
        ${poweredByHTML(config)}
      `;
  }).catch(() => {
    renderError(container, `Unable to load nutrition data for "${escapeHTML5(slug)}".`, config);
  });
}

// src/inline/macro-badge.ts
function initMacroBadgeInline(el, config) {
  var _a, _b, _c, _d;
  const ds = el.dataset;
  const cal = (_a = ds.calories) != null ? _a : "";
  const protein = (_b = ds.protein) != null ? _b : "";
  const carbs = (_c = ds.carbs) != null ? _c : "";
  const fat = (_d = ds.fat) != null ? _d : "";
  if (!cal && !protein && !carbs && !fat) return;
  el.setAttribute("data-inline", "");
  const shadow = createShadow(el, config);
  const container = createWidgetRoot(shadow, el);
  const parts = [];
  if (cal) parts.push(`<span style="font-weight:700;color:${config.accent};">${cal}</span> cal`);
  if (protein) parts.push(`<span style="font-weight:700;color:#3b82f6;">${protein}g</span> protein`);
  if (carbs) parts.push(`<span style="font-weight:700;color:#22c55e;">${carbs}g</span> carbs`);
  if (fat) parts.push(`<span style="font-weight:700;color:#f59e0b;">${fat}g</span> fat`);
  container.innerHTML = `
    <span style="display:inline-flex;align-items:center;gap:8px;padding:3px 10px;border-radius:6px;font-size:12px;background:color-mix(in srgb, ${config.accent} 6%, var(--bg));border:1px solid color-mix(in srgb, ${config.accent} 15%, transparent);">
      ${parts.join('<span style="color:var(--border);font-size:10px;">\xB7</span>')}
    </span>
  `;
}

// src/_entry_nutrifyi.ts
function initWidget(el, type, config) {
  ...
  switch (type) {
    ...
    case "faq":
      ...
      break;
    case "bmi-calculator":
      initBMICalculatorTool(el, config);
      break;
    case "nutrition-label":
      initNutritionLabelTool(el, config);
      break;
    case "macro-badge":
      initMacroBadgeInline(el, config);
      break;
    default:
      break;
  }
...
(function bootstrap() {
  const config = '{"site":"nutrifyi","name":"NutriFYI","domain":"nutrifyi.com","accent":"#22C55E","attribute":"data-nutrifyi","apiBase":"https://nutrifyi.com/api/v1/","searchPath":"/search/","entityName":"Foods","entitySlug":"foods"}';
...
function makeWidgetElement(widgetType, initFn, domainAttrs) {
  ...
  return class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this._syncDataAttrs();
      initFn(this, '{"site":"nutrifyi","name":"NutriFYI","domain":"nutrifyi.com","accent":"#22C55E","attribute":"data-nutrifyi","apiBase":"https://nutrifyi.com/api/v1/","searchPath":"/search/","entityName":"Foods","entitySlug":"foods"}');
    }
    ...
    _syncDataAttrs() {
      const attrKey = '{"site":"nutrifyi","name":"NutriFYI","domain":"nutrifyi.com","accent":"#22C55E","attribute":"data-nutrifyi","apiBase":"https://nutrifyi.com/api/v1/","searchPath":"/search/","entityName":"Foods","entitySlug":"foods"}'.attribute.replace("data-", "");
...
(function registerElements() {
  ...
  const site = '{"site":"nutrifyi","name":"NutriFYI","domain":"nutrifyi.com","accent":"#22C55E","attribute":"data-nutrifyi","apiBase":"https://nutrifyi.com/api/v1/","searchPath":"/search/","entityName":"Foods","entitySlug":"foods"}'.site;
  const defs = [
    ...
    [`${site}-bmi-calculator`, initBMICalculatorTool, []],
    [`${site}-nutrition-label`, initNutritionLabelTool, ["slug"]]
  ];
